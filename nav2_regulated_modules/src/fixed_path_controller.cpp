...
void FixedPathController::configure(const rclcpp_lifecycle::LifecycleNode::WeakPtr & parent, std::string name, std::shared_ptr<tf2_ros::Buffer> tf, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros) {
  ...
  if (!node) {throw nav2_core::PlannerException("FixedPathController cannot lock lifecycle node");}
  node_ = parent;
  tf_ = std::move(tf);
  ...
  if (base_linear_velocity_ <= 0.0 || lookahead_dist_ <= 0.0 || min_lookahead_dist_ <= 0.0 || max_lookahead_dist_ < min_lookahead_dist_ || start_position_tolerance_ <= 0.0 || initial_yaw_tolerance_ <= 0.0 || rotate_to_heading_angular_vel_ <= 0.0 || max_angular_accel_ <= 0.0 || approach_velocity_scaling_dist_ <= 0.0 || goal_position_hysteresis_ < 1.0 || alignment_stable_cycles_ < 1 || transform_tolerance_ < 0.0 || controller_frequency <= 0.0) {throw nav2_core::PlannerException("FixedPathController parameters are invalid");}
  speed_limit_ = base_linear_velocity_;
  control_duration_ = 1.0 / controller_frequency;
  LOG_INFO("固定路径控制器配置完成，plugin={}，最大线速度={:.3f}m/s，起点位置容差={:.3f}m，起点航向容差={:.3f}rad", plugin_name_, base_linear_velocity_, start_position_tolerance_, initial_yaw_tolerance_);
}

void FixedPathController::cleanup() {std::lock_guard<std::mutex> lock(mutex_); global_plan_ = nav_msgs::msg::Path(); nearest_index_ = 0; stable_cycles_ = 0;}

void FixedPathController::activate() {LOG_INFO("固定路径控制器已激活，plugin={}", plugin_name_);}

void FixedPathController::deactivate() {std::lock_guard<std::mutex> lock(mutex_); phase_ = Phase::SETTLE; stable_cycles_ = 0; LOG_INFO("固定路径控制器已停用，plugin={}", plugin_name_);}

void FixedPathController::setPlan(const nav_msgs::msg::Path & path) {
  std::lock_guard<std::mutex> lock(mutex_);
  if (path.poses.size() < 2 || path.header.frame_id.empty()) {throw nav2_core::PlannerException("Fixed path requires a frame and at least two poses");}
  std::size_t tangent_index = 1;
  while (tangent_index < path.poses.size() && poseDistance(path.poses.front(), path.poses[tangent_index]) <= 1e-6) {++tangent_index;}
  if (tangent_index >= path.poses.size()) {throw nav2_core::PlannerException("Fixed path has no valid tangent");}
  const double tangent_yaw = std::atan2(path.poses[tangent_index].pose.position.y - path.poses.front().pose.position.y, path.poses[tangent_index].pose.position.x - path.poses.front().pose.position.x);
  const double direction_cosine = std::cos(normalizeAngle(poseYaw(path.poses.front()) - tangent_yaw));
  if (!std::isfinite(direction_cosine) || std::abs(direction_cosine) < 0.5) {throw nav2_core::PlannerException("Fixed path orientation does not encode a clear direction");}
  global_plan_ = path;
  nearest_index_ = 0;
  direction_sign_ = direction_cosine > 0.0 ? 1 : -1;
  start_yaw_ = poseYaw(global_plan_.poses.front());
  goal_yaw_ = poseYaw(global_plan_.poses.back());
  stable_cycles_ = 0;
  phase_ = Phase::ALIGN_START;
  LOG_INFO("固定路径已装载，frame={}，路径点数={}，方向={}，起点航向={:.6f}rad，终点航向={:.6f}rad", global_plan_.header.frame_id, global_plan_.poses.size(), direction_sign_ > 0 ? "forward" : "backward", start_yaw_, goal_yaw_);
}
...
geometry_msgs::msg::TwistStamped FixedPathController::computeVelocityCommands(const geometry_msgs::msg::PoseStamped & pose, const geometry_msgs::msg::Twist & velocity, nav2_core::GoalChecker * goal_checker) {
  std::lock_guard<std::mutex> lock(mutex_);
  if (global_plan_.poses.size() < 2 || !goal_checker) {throw nav2_core::PlannerException("Fixed path controller has no valid plan or goal checker");}
  ...
  if (phase_ == Phase::ALIGN_START) {
    ...
    if (stable_cycles_ >= alignment_stable_cycles_) {
      phase_ = Phase::TRACK_PATH; stable_cycles_ = 0;
      LOG_INFO("固定路径起点航向对齐完成，方向={}", direction_sign_ > 0 ? "forward" : "backward"); return zeroCommand();
    }
...
bool FixedPathController::transformPose(const std::string & frame, const geometry_msgs::msg::PoseStamped & input, geometry_msgs::msg::PoseStamped & output) const {
  ...
  try {
    ...
    RCLCPP_ERROR(logger_, "Fixed path transform failed: %s", error.what()); return false;
  }
}
...
geometry_msgs::msg::TwistStamped FixedPathController::rotateCommand(const double yaw_error, const geometry_msgs::msg::Twist & velocity) const {
  ...
  if (std::abs(yaw_error) <= 1e-6) {return command;}
