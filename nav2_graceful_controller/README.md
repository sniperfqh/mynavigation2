...
Graceful Motion Controller 是一种几何运动控制器，通过连续控制律让机器人平滑地朝路径或目标运动。它根据机器人当前位姿、目标相对位置、曲率和速度约束计算线速度与角速度，并在接近目标时逐步减速，避免突然停车或剧烈转向。

配置部分定义控制增益、最大速度、最小速度、目标容差和减速半径等参数。Topics 部分列出控制器输出以及用于 RViz 调试的 Marker；slowdown Marker 表示以运动目标为中心、半径为 slowdown_radius 的平面圆。
