...
TEST(DynParamTestNode, testDynParamsSet) {
  ...
  auto parameter_client = std::make_shared<rclcpp::AsyncParametersClient>( node->shared_from_this(), "/test_costmap/test_costmap", rmw_qos_profile_parameters);
  auto results1 = parameter_client->set_parameters_atomically( { rclcpp::Parameter("robot_radius", 1.234), rclcpp::Parameter("footprint_padding", 2.345), rclcpp::Parameter("transform_tolerance", 3.456), rclcpp::Parameter("publish_frequency", 4.567), rclcpp::Parameter("resolution", 5.678), rclcpp::Parameter("origin_x", 6.789), rclcpp::Parameter("origin_y", 7.891), rclcpp::Parameter("width", 2), rclcpp::Parameter("height", 3), rclcpp::Parameter( "footprint", "[[-0.325, -0.325], [-0.325, 0.325], [0.325, 0.325], [0.46, 0.0], [0.325, -0.325]]"), rclcpp::Parameter("robot_base_frame", "test_frame"), });

  // Try setting robot_base_frame to an invalid frame, should be rejected
  auto results2 = parameter_client->set_parameters_atomically( { rclcpp::Parameter("robot_base_frame", "wrong_test_frame"), });
