...
Theta Star 是 Nav2 的任意角度全局规划器。它在栅格上使用 Lazy Theta* 变体和 Line-of-Sight 检查，把当前节点直接连接到父节点，减少传统 A* 的折线和不必要转弯，适合开放区域、地图走廊与需要快速规划的场景。

## 特性与代价函数

规划器支持二维 Costmap、任意角度路径、障碍代价和距离代价。代价函数通常由欧氏距离项和 Costmap 穿越项组成；当当前节点与候选邻居之间视线可通时，可以使用父节点路径代价加上父子距离和代价项，从而跳过中间栅格。参数包括欧氏代价权重、穿越代价权重、Costmap 阈值、容差、启发式和调试选项。

## 性能与使用建议

Metrics 用于比较规划时间、路径长度和搜索节点。代价地图膨胀、Costmap 分辨率和权重会同时影响路径安全性、平滑性和计算量。Theta Star 适合把规划速度放在极度平滑之前的场景，也适合地图图像中的走廊没有严格对齐栅格时生成近似直线的路线。若需要更高几何质量，可提高分辨率或在规划后使用路径平滑器，但应评估查询时间。
